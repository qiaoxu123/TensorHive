...
def generate_cert(path, replace=False):
    # Ed25519 instead of RSA-2048: libssh2 (used by parallel-ssh) only signs RSA keys
    # with SHA-1 (ssh-rsa), which OpenSSH >= 8.8 rejects by default. Ed25519 sidesteps this
    # so newly added nodes authenticate against modern sshd without any server-side changes.
    ...
    with open(str(path), 'wb') as f:
        f.write(openssh_pem)
    path.chmod(0o600)
    return Ed25519Key.from_private_key_file(str(path))


def _load_key(path: str):
    '''Load an existing TensorHive SSH key, supporting both Ed25519 (current) and
    legacy RSA keys generated by older TensorHive versions.'''
    try:
        return Ed25519Key.from_private_key_file(path)
    except Exception:
        return RSAKey.from_private_key_file(path)


def init_ssh_key(path: PosixPath):
    if path.exists():
        key = _load_key(str(path))
